...
/** Example test */
public class UserManagedLockManagerBasedLockingTest1 {
    ...
    @TestEngine.Test
    public void lockingTest() throws Throwable {
        System.out.println("lockingTest()");

        context.getLockManager(NAMESPACE).getLock(LOCK_NAME).writeLock().lock();
        ...
        try {
            ...
            context.getLockManager(NAMESPACE).getLock(LOCK_NAME).writeLock().unlock();
        }

        context.getLockManager()
                .executeInWriteLock(
                        "TEST",
                        () -> {
                            System.out.println("sleeping UserManagedLockManagerBasedLockingTest1");
                            Thread.sleep(10000);
                            System.out.println(
                                    "continuing UserManagedLockManagerBasedLockingTest1");
                        });
    }
